...
### Why unsupervised anomaly detection instead of a supervised classifier

All three datasets actually have a label column (`is_attack`, `label`, `class`) — so a fair question is why not just train a normal supervised classifier on it instead. The answer is the same reason mentioned above under train/test splitting: the entire point of anomaly detection is catching things that haven't been seen before. A supervised model only ever learns to recognize the *specific* attack patterns it was shown labeled examples of — a genuinely new attack style that doesn't resemble any labeled training example gives it no reason to flag anything, it just confidently predicts whatever it's closest to. Isolation Forest doesn't need to have seen a specific attack before; it only needs the new attack to be structurally rare relative to the crowd of normal behavior. That's a more useful property for security specifically: catching the *unknown*, not just recognizing the *known*. It also mirrors a real practical constraint even though it doesn't fully apply inside this project's toy data: real production log data is rarely exhaustively labeled, so a method that doesn't depend on that is more realistically deployable. That's why the label columns are only ever used for `y_test` comparison after the fact, never fed into `.fit()`.

**The honest trade-off, learned the hard way during cloud's Phase 7:** a supervised model wouldn't have hit the retraining-backfire problem described in the adversarial testing section below — labeled adversarial examples would directly teach "predict attack for this shape." Isolation Forest's unsupervised nature is exactly *why* naive retraining backfired instead. Worth framing this honestly for the patent: not "we picked the easier option," but "we deliberately chose the harder-to-harden method because it's the right one for catching novel attacks, and adversarial evaluation surfaced a real, worth-publishing trade-off" — unsupervised models need genuine feature engineering to harden against evasion, not just more data thrown at retraining.
...
In this codebase's convention, `decision_function` is **higher = more normal** (every `train_detector.py` has a comment saying exactly this). So for a genuinely anomalous row, replacing the feature that's actually *causing* the anomaly with a normal/average value should push the score *up* (toward normal) — the informative deltas are large and positive (`new_score - baseline > 0`). The feature search needs to find the *largest absolute change*, `max(abs(new_score - baseline))`, not just the largest signed change in one specific direction — because if a search only looks for the most-negative delta (`max(baseline - new_score)`, which is the same as the most-negative `new_score - baseline`), it's specifically searching for the feature that makes the row *more* anomalous when neutralized — the opposite of a feature that explains the anomaly — and most features don't do that at all, so a search restricted to that direction ends up picking whichever feature had the smallest, most negligible effect, while missing the actual dominant feature entirely.

Checked this empirically on 5 real flagged cloud rows: the signed version and the absolute-value version picked a **different feature every single time**, and the truly-dominant feature (found by the absolute-value search) moved the score 10-100x more than whatever the signed search had settled on. That's not a rounding difference — it means a search restricted to one sign direction can silently produce a `top_feature` that's nearly irrelevant to the row's real anomaly cause, without ever throwing an error or looking obviously wrong (it still returns *a* feature name, just not the *right* one). Worth double-checking against `abs()` before trusting any `top_feature` value downstream — including feeding it into an LLM prompt, the way Devyani's Phase 12 does, where a wrong attribution would actively mislead the explanation rather than just being a cosmetic error in a spreadsheet somewhere.
...
**Why retrain instead of just reporting the evasion rate:** measure how many evaded attacks still get caught (the "before defenses" baseline), then retrain with some of those adversarial examples included in the training data, then re-run the exact same evasion attempts (the "after defenses" number). The intent is that those specific altered feature combinations now actually appear in training, so Isolation Forest's random splitting process naturally includes splits that isolate them too — but this isn't guaranteed, and it's worth checking, not assuming. See below.

**A real failure mode: naive retraining can make an unsupervised model *worse*, not better.** Isolation Forest has no labels — the only thing that makes it call a point anomalous is how sparse its neighborhood is. Adding "adversarial" rows to training doesn't tell the model "distrust this shape," the way a labeled example would for a supervised classifier — it only ever makes that corner of feature space more crowded. If that corner already had *legitimate* data living in it (e.g. a small number of real, non-malicious rows sharing the same feature combination as the evasion attempt), adding more points there makes the whole region denser, which lowers the anomaly score for everything sitting there — including the very evasion rows retraining was supposed to catch. Concretely: cloud detector retraining (Phase 7) dropped mimicry-evasion detection from 15/30 to 6/30 after adding 30 adversarial examples, precisely because those examples landed in the same feature corner as the legitimate admin-user rows from the Phase 5 redesign, thickening that crowd rather than teaching the model to distrust it. Not a scale artifact either — this happened on a ~4,400-row training set, not a huge one. Worth checking for on any of the three detectors: does the "after" number actually improve, or does it move the wrong way? If retraining doesn't help, that itself is a real, reportable finding, not something to force into looking better.

**The fix isn't more data, it's better features — and that lands differently per layer.** Since Isolation Forest can only ever separate two situations using the features it's actually given, retraining can't manufacture a distinction that doesn't exist in `X` — the real fix is checking whether some genuinely discriminating signal exists in the raw data but was never engineered into a feature at all. Whether that fix is actually *available* differs a lot by layer — see each person's section below.

**What happened when that fix was actually tried, on cloud:** added `is_unusual_hour` (real attacks always happen 1am-4am, legitimate rows never do) as a third feature, and upgraded the mimicry attack so the attacker fakes *both* signals instead of one. That confirmed the feature genuinely works — faking only the IP is now trivially caught, since the untouched timestamp gives it away — but once the attacker fakes every currently-modeled signal together, retraining backfired again, the same way, just one level deeper. At that point the attacker's row becomes **literally identical**, feature-for-feature, to a legitimate admin row — there is no split any tree could ever make to separate two rows with identical values on every column, because a split only exists where rows differ on something. This isn't a bug to keep chasing with more features; it's a real, general ceiling: **behavioral-log features alone can only ever distinguish two situations that actually differ somewhere in the log.** If an attacker can observe and copy every signal a detector uses, no model — supervised or unsupervised — can separate them on those features. Getting past that requires a signal the attacker structurally can't replicate (device/session fingerprinting, MFA context), not more retraining or more log-derived features. Worth checking for on any layer: if an evasion attempt still succeeds after every available raw-data signal has been engineered into a feature, that's this same ceiling, not a modeling gap.
...
**Fixed.** `get_top_feature()` in `detectors/network_detector/train_detector.py` used to pick the feature maximizing `baseline - new_score` (a signed search), which finds the feature whose neutralization makes a row *more* anomalous, not the one that actually explains why it's anomalous. Now picks the feature maximizing `abs(new_score - baseline)` instead, matching cloud's `train_detector.py`. Hridya reran with the fix and pushed the corrected `flags.json` — attribution now spreads across 48 distinct features instead of being dominated by one. See "Leave-one-feature-out attribution" in Everyone's section above for the full reasoning, and `results/cloud_llm_metrics.md` for the before/after comparison.
...
Concretely, on the real data (as of the Sept push, after Hridya/Anshika's Phase 9 scenario legs and my Phase 10 cloud legs): out of 650 total chains fusion finds, only the deliberately-planted scenario rows touch cloud at all, and 15 chains span more than one layer. Of those, exactly 3 span all three layers — `incident_demo_01` plus the two new genuine scenarios, `chain_apt_02` and `chain_cred_03` — every one of them a scenario we planted on purpose, never something that emerged organically from the real data. Zero *organic* three-layer chains exist. This is a real structural gap worth stating plainly: cloud's identity namespace (`user1..user30`/ARNs) structurally can never collide with network/OS's shared `USR-XXX`/`HOST-XXX` scheme, so no amount of tuning the fusion logic itself would ever produce a real three-layer chain from the current data — it needs an actual identity-resolution layer between cloud and the other two, not a fusion-matching fix.

**A more serious problem, found while validating Phase 10's precision/recall on the *full* flagged output rather than just the scenario rows:** the single largest chain contains 20,435 records — about 19% of all combined network+OS flagged rows — spanning every one of network's 25 simulated hosts and 25 simulated entities. Checking the full chain-size distribution (done properly in Phase 11, see `results/cloud_llm_metrics.md`) found this isn't a one-off: there are 10 chains this large, together absorbing 95.7% of every flagged record in the dataset. This isn't a real incident; it's `fuse.py`'s transitive linking (if A links to B, and B separately links to C, then A and C end up in the same chain even though they never co-occurred) combining with how network assigns `entity`/`host`: independently at random per flagged row, from a pool of just 25 values each (`random.choice(users)` / `random.choice(hosts)` per row in `train_detector.py`, since raw CICIDS2017 flow data has no per-flow user identity to draw a real one from). With ~106K flagged rows and only 25 possible host values, the same host gets reused thousands of times purely by chance, and enough of those chance reuses land within the 10-minute window to transitively chain the *entire* flagged population into one blob. OS and cloud don't have this problem, because their identity assignment is actually tied to something (OS ties `entity`/`host` to the row's real simulated user; cloud gives each user a fixed "home" identity) — which is exactly why the mega-chain is 99.87% network rows with only 26 OS rows swept in at the edges, and zero cloud. Full writeup, and the specific flag to Hridya about her identity-assignment code, is in `results/cloud_llm_metrics.md` under "Fusion chain-reconstruction accuracy."

**The general lesson, worth telling Hridya and Anshika too even though the concrete bug is network-specific:** transitive identity-linking (the kind fusion does, and the kind any real-world alert-correlation system does) is only as sound as the assumption that a shared identifier actually means "the same real thing happened twice." The moment an identifier gets assigned independently at random rather than tied to something real and stable, transitive closure turns "harmless coincidental reuse" into "everything eventually connects to everything," and the failure mode doesn't show up as an error — it shows up as one suspiciously huge chain that's easy to miss if you only check summary counts (`len(attack_chains)`) instead of looking at what's actually inside the biggest one.
...
**Grounding the explanation in feature attribution, not just identifying metadata (Phase 12).** Up through Phase 9, the LLM only ever saw `entity`/`host`/`timestamp`/`anomaly_score`/`layer` — enough to say *who, where, when, how suspicious*, but nothing about *why* the detector actually flagged that row. Once every detector's `flags.json` carries a `top_feature` (see Everyone's leave-one-feature-out section above), `SYSTEM_INSTRUCTION` was updated to explicitly reference it per record, and the model's summaries changed from generic ("anomalous network activity was detected on this host") to specific and mechanistic ("Flow IAT Max was far outside the normal range, typical of a data-exfiltration burst or C2 beacon"). Confirmed by building a demo chain out of three *real* flagged rows (one per layer, each with a genuine non-null `top_feature`) rather than the usual `incident_demo_01` — that hand-planted row has `top_feature: null` on all three of its legs, since it's a synthetic coordination row with no real feature vector behind it, so it can't actually demonstrate this feature working.
...
**Real result, quantified (Phase 9):** originally tested with one hand-crafted example; a single example isn't a rate, so this got expanded to 20 varied attempts (varying phrasing, which field carries the injection, and what it tries to force) run through both the vulnerable and defended prompts. **25% (5/20) succeeded before the defense, 0% (0/20) after.** The pattern in *which* attempts succeeded is the interesting part: blunt "SYSTEM OVERRIDE: ignore all instructions" phrasing failed even without any defense — modern models already resist that pattern from their own safety training — while subtler, social-engineering-style framing (claiming prior authorization, claiming the incident was already reviewed and cleared, citing a fake policy) is what actually got through undefended. The defense rule closed all 5 anyway, confirming it works as a structural "distrust any claim in the evidence" rule rather than pattern-matching against obvious command phrasing. (One infrastructure note behind this: Groq deprecated the original model mid-project, forcing a switch to a different one — disclosed clearly in the results writeup since it means the "before" percentage isn't perfectly comparable to the original single-example result, though the "after" comparison still is.)
