...
**Resolved.** A sign error was found in network's leave-one-feature-out method (`max(baseline - new_score)`, a signed search, rather than `max(abs(new_score - baseline))`, a magnitude search). Tested on 5 real cloud rows: the two methods picked a **different feature every time**, with the true dominant feature moving the score 10-100x more than whatever the signed version had settled on. Checked directly (not assumed) that this was network-specific: Anshika's code already used the correct absolute-value approach independently, so OS's values were never affected. Hridya pulled the fix, reran her detector, and pushed the corrected `flags.json` (commit `7e0c069`) — the new distribution spreads across 48 distinct features (max 171/1000 rows sharing one), a healthy spread rather than the degenerate pattern the signed search produced. Objective 3's grounding claim ("the specific features that drove each detection") now holds across all three layers.
