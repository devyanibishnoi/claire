...
### Important flag for Hridya: network's leave-one-feature-out method has an inverted sign (Anshika's does not)
...
- **Signed-max (what `network_detector`'s current code does):** `drop = baseline - new_score`, then pick the feature maximizing `drop`. This selects the feature whose neutralization makes the row score *more* anomalous, however slightly.
- **Absolute-max (what cloud's new code does, and what `os_detector`'s code already did):** pick the feature maximizing `abs(new_score - baseline)`, regardless of direction.
...
**What this means practically, checked directly rather than assumed:** the checklist told Anshika to copy "the same method as Hridya's Phase 8," but her actual code (`find_top_feature` in `detectors/os_detector/train_detector.py`) already computes `abs(modified_score - original_score)` and picks the max via that — the correct approach, independently arrived at, matching cloud's fix exactly. **Her `top_feature` values don't need anything changed.** Only network's `top_feature` values are affected and need re-running with the sign fixed before they get used anywhere that matters, including feeding into an LLM prompt the way Phase 12 does for cloud. This is exactly the kind of thing Phase 12 exists to catch: grounding the LLM's explanation in attribution data is only as good as the attribution data actually being right.
...
### Generalization, fusion, and impact (Phase 8, 10, 11)

| Result | Value | Notes |
|---|---|---|
| Cross-paradigm generalization | LOF fails to establish a usable baseline (0/105 real attacks) | Root cause: exact-duplicate degeneracy in a purely categorical feature space, not a paradigm-robustness difference. See "Cross-paradigm generalization" above for the full diagnostic (jitter test, `n_neighbors` sweep). Since LOF never clears the baseline bar, the retraining-backfire comparison couldn't be repeated on it directly, an answer to Objective 6 in its own right (paradigm choice must match feature-space shape). |
| Fusion recall / precision, controlled scenarios | 3/3 genuine chains reconstructed, 0/2 decoy scenarios falsely merged (100% / 100%) | Small-n by design (a constructed validation set, not a statistical sample); both decoy variants covered, entity/host mismatch and time-window mismatch. |
| Fusion at full scale, unplanned finding | 10 chains (95.7% of all network+OS flagged records) collapse into artifact "mega-chains" | Traced to network's identity assignment (`entity`/`host` drawn independently at random per row from only 25 possible values), not a fusion-logic bug. Flagged to Hridya directly, with a concrete fix direction. |
| Alert-reduction ratio | 97.14% naive / **1.45% artifact-corrected** / **70.76% fusion-only** | Reported all three rather than the flattering one alone: the naive number is mostly the mega-chain artifact above collapsing, not real correlation value. 70.76% is what fusion's own logic delivers on records it can evaluate soundly. |
...
- Cloud repeated the experiment on a second algorithm family (`LocalOutlierFactor`) and found a deeper, structural nuance: the paradigm itself (global partition-based vs. local distance-based) has to match the shape of the feature space, independent of the adversarial question. See the generalization table above.
...
### Explanation grounded in cause, not just identity (Phase 12)

The LLM explanation layer now references each event's `top_feature` (the single feature that drove its anomaly score) directly in its narrative, confirmed on a demo chain built from real, non-null attribution values across all three layers. This closes Objective 3's "grounded in the specific features that drove each underlying detection rather than in identifying metadata alone."

**Needs Hridya's attention before this goes in the paper as a finished claim:** while implementing cloud's own `top_feature` attribution, a sign error was found in network's leave-one-feature-out method (`max(baseline - new_score)`, a signed search, rather than `max(abs(new_score - baseline))`, a magnitude search). Tested on 5 real cloud rows: the two methods picked a **different feature every time**, with the true dominant feature moving the score 10-100x more than whatever the signed version had settled on. **Checked directly, not assumed:** although the checklist told Anshika to copy the same method, her actual code already uses the correct absolute-value approach, so this affects only network's already-pushed `top_feature` values, not OS's. See "Grounding the LLM explanation in feature attribution" above for the full comparison. **Action needed:** Hridya should re-check her `get_top_feature()` against the absolute-value version before Objective 3's grounding claim is stated as fully verified across all three layers in the paper. No action needed from Anshika on this one.
